...
contract asset {
	...
	function createProperty(uint _propId, uint _value,string _locatity, address _owner) verifiedAdmin verifiedUser(_owner) returns (bool) {
		properties[_propId] = PropertyDetail(Status.Pending, _value, _locatity,_owner);
		return true;
	}
